...
## 2026-04-10
>>>>>>> 763b0638233b34d0f7bc63a042aae7fa780333dc
...
- 清理了一批明确无业务依赖的生成产物与临时文件：
  - 删除 `.tmp/pyclobv2/` 第三方依赖解包目录
  - 删除 `.tmp_ref_images/openalice-preview.png` 临时参考图
  - 删除 `logs/seed_tagging_defaults_20260419.json` 及当前运行日志文件
  - 删除 `apps/web/tsconfig.tsbuildinfo` 类型检查缓存
- 更新 `.gitignore`，补充忽略：
  - `.tmp/`
  - `.tmp_ref_images/`
  - `*.tsbuildinfo`
  - `logs/**`（保留 `logs/.gitkeep`）

### 说明

- 这次清理只处理“可再生的临时产物”，没有删除业务源码、接口、测试或正式文档。
>>>>>>> 763b0638233b34d0f7bc63a042aae7fa780333dc
